...
;; Maintainer: John Charman <johnrcharman@gmail.com>
...
For Godot 4.1 onwards, VERSION is the '4.1', '4.2', etc."
  (with-temp-buffer
	...
	(add-to-list 'settings `(indent-tabs-mode ,(if (equal gdscript-use-tab-indents 'auto)
				                    (gdscript-editor-settings--indent-type)
                                                  (gdscript-use-tab-indents)))))
      (unless (equal gdscript-tab-width 'ignore)
	(add-to-list 'settings `(tab-width ,(if (equal gdscript-tab-width 'auto)
                                               (gdscript-editor-settings--indent-size)
                                             (gdscript-tab-width)))))
      (add-to-list 'settings `(gdscript-use-type-hints ,(if (equal gdscript-use-type-hints 'auto)
